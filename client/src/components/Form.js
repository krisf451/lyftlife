...
const Form = ({ currentId, setCurrentId }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (currentId) {
      dispatch(updateAsyncWorkout({ ...formValues, name: user?.result?.name }));
    } else {
      dispatch(postAsyncWorkout({ ...formValues, name: user?.result?.name }));
    }
    clear();
  };
    ...
    return (
      <div>
        <h6 className="text-center">
          Please Sign In to create your own workouts and like others workouts
        </h6>
      </div>
    );
  }
